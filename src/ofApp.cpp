    ...
    if (kinect.isFrameNew()) {
        ...
        if (contourFinder.bTrackFingers) {
            for (int i = 0; i < contourFinder.fingers.size(); i++) {
                ...
                contourFinder.fingers[ i ].centroid.y =
                    kinect.getDistanceAt(pos);
            }
        }
...
void ofApp::draw() {
    ...
    grayImage.draw(MAIN_WINDOW_WIDTH + 20, 10, MAIN_WINDOW_WIDTH,
                   MAIN_WINDOW_HEIGHT);
    ...
    if (contourFinder.bTrackBlobs) {
        for (int i = 0; i < contourFinder.nBlobs; i++) {
            ...
            if (bShowLabels)  // Show ID label
            {
                ...
                verdana.drawString(
                    idStr, xpos + MAIN_WINDOW_WIDTH + 20,
    ...
    reportStream << "using opencv threshold = " << bThreshWithOpenCV
                 << " (press spacebar)" << endl
                 << "set near threshold " << nearThreshold << " (press: + -)"
                 << endl
...
bool ofApp::loadXMLSettings() {
    ...
    if (XML.loadFile("config.xml")) {
        if (XML.pushTag("CONFIG")) {
            ...
            contourFinder.bTrackFingers =
                XML.getValue("BOOLEAN:TRACKFINGERS", 0);
            contourFinder.bTrackObjects =
                XML.getValue("BOOLEAN:TRACKOBJECTS", 0);
